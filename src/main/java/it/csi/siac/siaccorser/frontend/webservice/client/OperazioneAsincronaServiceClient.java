...
/**
 * Client del servizio OperazioneAsincronaServiceClient
 * 
 * @author rmontuori
 * 
 */
public class OperazioneAsincronaServiceClient extends BaseServiceClient {

	private OperazioneAsincronaService port;

	public OperazioneAsincronaServiceClient(){
		super();
	}
	...
	private void createService() {
		try {
			...
			serviceName = new QName(CORSvcDictionary.NAMESPACE,
					"OperazioneAsincronaService");
			service = Service.create(wsdlURL, serviceName);
			port = service.getPort(OperazioneAsincronaService.class);
	...
	public OperazioneAsincronaService getPort() {
		if (service==null)
			createService();
